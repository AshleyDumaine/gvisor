...
func (*lisafsDentry) revalidateHelper(ctx context.Context, vfsObj *vfs.VirtualFilesystem, state *revalidateState, ds **[]*dentry) error {
	// Re-calculate start *lisafsDentry, instead of using the function receiver.
	// The function receiver has to be `d`, which is also used below in various
	// places. Makes programming this less error prone and code more readable.
